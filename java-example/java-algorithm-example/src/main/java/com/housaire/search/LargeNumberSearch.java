...
//    static int max = 1000;
//    static int range = 8;
//    static int maxBit = 0x7f;
//    static byte[] bytes = new byte[max / range];
//
...
//        long beginTime = System.currentTimeMillis();
//
//        List<Integer> searchNums = new ArrayList<>();
//        int index;
//        byte fixed = 1;
//        for (int i = 1; i < max - range; i++)
//        {
//            if (nums.contains(i))
//            {
//                searchNums.add(i);
...
//            }
//            int v = i - (index * range);
//            bytes[index] = (byte) (bytes[index] | ((fixed << (v - 1)) & maxBit));
//        }
...
//
//            index = searchNums.get(i) / range; // 获得byte下标
//            int n = (searchNums.get(i) - (index * range)) - 1;
//            boolean exist = ((((1 << n) & maxBit) & bytes[index]) == n);
//            System.err.println("【 " + exist + " 】 - 数据 [ "
//                    + searchNums.get(i) + " ] 查找耗时：" + (System.currentTimeMillis() - beginTime)
//                    + " 毫秒, 所在下标：" + index + "  下标值为：" + bytes[index]);
//        }
//    }
}
