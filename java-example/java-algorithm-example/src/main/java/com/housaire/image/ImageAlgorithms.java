...
//			m.insert(p);
    ...
    static void encode() throws IOException {
        String orignImg = "H:\\imageed\\raw.jpg";
        String encryptImg = "H:\\imageed\\out2.jpg";
        BufferedImage image = ImageIO.read(new File(orignImg));
        int w = image.getWidth();
        int h = image.getHeight();
        int[] pixels = null;
        pixels = image.getRaster().getPixels(0, 0, w, h, pixels);

        int pixelX = pixels.length / w;
        int pixelY = pixels.length / pixelX;

        int[][] pixelsArray = new int[pixelX][pixelY];
        ArrayFunctions af = new ArrayFunctions();
        af.change(pixels, pixelsArray, pixelX, pixelY);

        ImageAlgorithms algorithms = new ImageAlgorithms();
        algorithms.encrypt(pixelsArray, 123456, pixelX, pixelY);
        System.out.println(pixelsArray.length);
        af.recovery(pixelsArray, pixels, pixelX, pixelY);
        BufferedImage imageEncrypt = new BufferedImage(w, h, image.getType());
        imageEncrypt.setRGB(0, 0, w, h, pixels, 0, w);
        ImageIO.write(imageEncrypt, "jpg", new File(encryptImg));
        image.flush();
    }

    static void decode() throws IOException {
        String orignImg = "H:\\imageed\\out.jpg";
        String decryptImg = "H:\\imageed\\new.jpg";
        BufferedImage image = ImageIO.read(new File(orignImg));
        int w = image.getWidth();
        int h = image.getHeight();
        int[] pixels = null;
        ...
        int[][] pixelsArray = new int[pixelX][pixelY];
        ArrayFunctions af = new ArrayFunctions();
        af.change(pixels, pixelsArray, w, h);

        ImageAlgorithms algorithms = new ImageAlgorithms();
        algorithms.decrypt(pixelsArray, 123456, pixelX, pixelY);
        System.out.println(pixelsArray.length);
        af.recovery(pixelsArray, pixels, pixelX, pixelY);


        BufferedImage imageDecrypt = ImageIO.read(new File(decryptImg));
        imageDecrypt.getRaster().setPixel(0, 0, pixels);
    }
